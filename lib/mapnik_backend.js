...
MapnikSource.prototype._createPool = function(xml, callback) {
    var source = this;
    if (!this._pool) {
        var poolFactory = this._uri.poolFactory || {};
        poolFactory.create = function(callback) {
        ...
        poolFactory.destroy = function(map) {
            delete map;
        };
        // @TODO: need a smarter way to scale this. More
        // maps in pool seems better for PostGIS.
        if (!('max' in poolFactory))
            poolFactory.max = require('os').cpus().length;
        this._pool = Pool(poolFactory);
    }
    callback(null);
};
